...
- 用户新增明确要求：以后汇报开发进度时，还要额外说明本轮代码增加/减少的行数，以及增加/减少的文件数量。
- 用户新增明确要求：以后汇报开发进度时，还要额外说明本次 Codex 实际运行了多久（开始到结束时长，或当前已运行多久）；即使任务已经跑完，也要把这次总时长明确报出来，不能漏掉。
- 用户明确纠正：不要把内部工作说明、内部检查清单或英文自言自语样的过程文本直接回给老板；对外只发整理过的正式答复。
- 用户明确纠正：在有 reply_to（回复上下文）时，不能只看消息正文里像“继续开发”这种短句就默认推进老任务；必须优先按回复目标理解语境，若上下文冲突就先澄清，不要误把别的消息当成这条消息的意图。
...
- 用户再次明确纠正的流程点：只要发生启动失败，Claw 必须先主动汇报，再立即执行既定三选一（重试另一条路径 / 切备用路径 / 明说当前无 active executor）；不能既不汇报，也不重启，让老板后来追问才知道。
