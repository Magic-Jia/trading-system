...
- That fallback status update must say explicitly: `Codex: running / not running`, a status type of `started / start_failed / stopped`, child-session status when applicable, whether there is a new commit, the latest verified command/result, the current blocker (if any), and the next action. If Codex is not running, say whether it exited, failed to start, crashed, or there is no active executor.
