...
def test_load_raw_market_manifest_rejects_out_of_order_file_timestamps(tmp_path: Path) -> None:
    archived = archive_raw_market_payload(
        archive_root=tmp_path / "archive",
        exchange="binance",
        market="futures",
    ...
    with pytest.raises(ValueError, match="raw-market record timestamps must be strictly increasing"):
        load_phase1_raw_market_manifest(archived.manifest_path)
...
def test_series_report_rejects_non_lowercase_64_hex_provenance_sha256(tmp_path: Path, sha256: str) -> None:
    archived = archive_raw_market_payload(
        archive_root=tmp_path / "archive",
        exchange="binance",
        market="futures",
    ...
    bad_file = raw_file.__class__(
        ...
        manifest_path=raw_file.manifest_path,
        data_path=raw_file.data_path,
        manifest={**raw_file.manifest, "file": {"sha256": sha256}},
        symbol_metadata=raw_file.symbol_metadata,
        coverage_start=raw_file.coverage_start,
        coverage_end=raw_file.coverage_end,
        fetched_at=raw_file.fetched_at,
    ...
    with pytest.raises(ValueError, match="raw-market provenance sha256 must be lowercase 64-hex"):
        _series_report(bad_series, expected_interval=None)
...
def test_load_raw_market_manifest_rejects_metadata_symbol_identity_drift(tmp_path: Path) -> None:
    archived = archive_raw_market_payload(
        archive_root=tmp_path / "archive",
        exchange="binance",
        market="futures",
    ...
    manifest = json.loads(archived.manifest_path.read_text(encoding="utf-8"))
    manifest["metadata"]["symbol"] = "ETHUSDT"
    archived.manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("coverage_end", "2026-01-01 02:00:00Z"),
        ("fetched_at", "1767232860000"),
    ],
)
def test_load_raw_market_manifest_rejects_noncanonical_manifest_timestamps(
    tmp_path: Path,
    field: str,
    value: str,
) -> None:
