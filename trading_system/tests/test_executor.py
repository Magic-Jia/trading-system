...
def test_order_executor_testnet_rejects_non_string_entry_status_before_protection(monkeypatch, tmp_path):
    from trading_system.app.execution import executor as executor_module
    ...
    with pytest.raises(Exception, match="entry order status must be a string"):
        executor.execute(order, RuntimeStateV2.empty())
