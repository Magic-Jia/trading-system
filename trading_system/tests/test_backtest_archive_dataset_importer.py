...
@pytest.mark.parametrize(
    ("field", "value", "expected_message"),
    [
        ("notional", "30000.0", r"account\.open_positions\[0\]\.notional must be a non-negative finite number"),
        ("notional", -1.0, r"account\.open_positions\[0\]\.notional must be a non-negative finite number"),
...
def test_write_phase1_dataset_bundle_rejects_malformed_open_position_risk_prices_without_artifact(
    ...
    expected_qualifier = "positive finite" if field == "risk_price" else "non-negative finite"
    with pytest.raises(
        ValueError,
