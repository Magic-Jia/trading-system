...
def test_collect_trade_outcomes_rejects_non_string_allocation_status(tmp_path: Path) -> None:
    paths = build_runtime_paths("paper", runtime_root=tmp_path / "runtime", runtime_env="research")
    module = _outcomes_module()

    with pytest.raises(ValueError, match="fact.allocation_status must be a string"):
        module.collect_trade_outcomes(
            trade_outcomes_path=paths.trade_outcomes_file,
            signal_facts=[
                {
...
def test_collect_trade_outcomes_rejects_non_string_position_status(tmp_path: Path) -> None:
    paths = build_runtime_paths("paper", runtime_root=tmp_path / "runtime", runtime_env="research")
    module = _outcomes_module()

    with pytest.raises(ValueError, match="position.status must be a string"):
        module.collect_trade_outcomes(
            trade_outcomes_path=paths.trade_outcomes_file,
            signal_facts=[
                {
...
def test_collect_trade_outcomes_rejects_non_string_position_opened_at(tmp_path: Path) -> None:
    paths = build_runtime_paths("paper", runtime_root=tmp_path / "runtime", runtime_env="research")
    module = _outcomes_module()

    with pytest.raises(ValueError, match="position.opened_at_bj must be a string"):
        module.collect_trade_outcomes(
            trade_outcomes_path=paths.trade_outcomes_file,
            signal_facts=[
                {
