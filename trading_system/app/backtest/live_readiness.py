...
TRADE_BOUNDED_COST_RATIO_FIELDS = (
    "fee_pct",
    "slippage_pct",
    "funding_pct",
    "cost_pct",
)
TRADE_BOUNDED_COST_BPS_FIELDS = (
    "fee_bps",
    "slippage_bps",
    "funding_bps",
    "cost_bps",
...
def _validate_postmortem_trade_execution_fields(trade: Mapping[str, Any], index: int) -> None:
    ...
    value = trade.get("liquidity_role")
    if value is not None and (
    ...
    for field in TRADE_BOUNDED_COST_RATIO_FIELDS:
        value = trade.get(field)
        if value is not None:
            parsed, valid = _strict_float_value(value)
            if not valid or parsed < 0.0 or parsed > 1.0:
                raise ValueError(
                    f"postmortem.trades[{index}].{field} must be a bounded non-negative cost ratio strict number"
                )
    for field in TRADE_BOUNDED_COST_BPS_FIELDS:
        value = trade.get(field)
        if value is not None:
            parsed, valid = _strict_float_value(value)
            if not valid or parsed < 0.0 or parsed > 10000.0:
                raise ValueError(
                    f"postmortem.trades[{index}].{field} must be bounded non-negative finite strict cost bps"
                )
        ...
        if reconciliation:
            lines.extend(
                [
                    ...
                    f"- matched: {str(_strict_report_bool(reconciliation, 'matched')).lower()}",
                    f"- gate_trade_count: {_strict_bucket_int(reconciliation, 'gate_trade_count')}",
                    f"- postmortem_trade_count: {_strict_bucket_int(reconciliation, 'postmortem_trade_count')}",
                    f"- trade_count_delta: {_strict_bucket_int(reconciliation, 'trade_count_delta')}",
                    f"- gate_net_pnl: {_strict_bucket_float(reconciliation, 'gate_net_pnl'):.2f}",
                    f"- postmortem_net_pnl: {_strict_bucket_float(reconciliation, 'postmortem_net_pnl'):.2f}",
                    f"- net_pnl_delta: {_strict_bucket_float(reconciliation, 'net_pnl_delta'):.2f}",
                    "",
                    "### Failure Taxonomy",
                ]
            )
    ...
    if runtime_safety:
        ...
        lines.extend(
            [
                ...
                f"- required: {_format_strict_bool(runtime_safety, 'required')}",
                f"- artifact_count: {_strict_bucket_int(runtime_safety, 'artifact_count')}",
    ...
    if microstructure:
        ...
        lines.extend(
            [
                ...
                f"- required: {_format_strict_bool(microstructure, 'required')}",
                f"- artifact_count: {_strict_bucket_int(microstructure, 'artifact_count')}",
    ...
    if validation:
        ...
        lines.extend(
            [
                ...
                f"- required: {_format_strict_bool(validation, 'required')}",
                f"- artifact_count: {_strict_bucket_int(validation, 'artifact_count')}",
