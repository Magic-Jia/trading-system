...
def _json_object_field(value: Any, *, context: str) -> dict[str, Any]:
    ...
    for key in value:
        if not isinstance(key, str) or not key.strip() or key != key.strip():
            raise ValueError(f"{context} keys must be canonical strings")
    return dict(value)
...
def _validate_material_derivatives_snapshot_identity_fields(derivatives_snapshot: Mapping[str, Any]) -> None:
    rows = derivatives_snapshot.get("rows") if "rows" in derivatives_snapshot else ()
    if rows is None:
        return
    if not isinstance(rows, Sequence) or isinstance(rows, (str, bytes, bytearray)):
    ...
    for index, row in enumerate(rows):
        ...
        for field_name in ("symbol", "instrument", "category", "exchange", "contractType"):
            if field_name not in row:
                continue
            field = f"derivatives_snapshot rows[{index}].{field_name}"
            value = _require_canonical_string(row.get(field_name), field=field)
...
def _validate_material_instrument_snapshot_rows(rows: Sequence[Mapping[str, Any]]) -> None:
    for index, row in enumerate(rows):
        for field in ("quote_volume_usdt_24h", "quantity_step", "price_tick", "min_notional", "lot_size"):
            if field not in row:
                continue
            value = row[field]
...
def _archive_root_from_manifest_paths(manifest_paths: Sequence[str]) -> Path | None:
    ...
    for value in _require_canonical_string_items(manifest_paths, field="source manifest_paths entries"):
        ...
        if ".." in manifest_path.parts:
            raise ValueError("source manifest_paths entries must not contain parent traversal")
    ...
    *,
    ...
    if root_manifest is not None:
        ...
        manifest_archive_root = _resolved_phase1_imported_dataset_root_path(
            dataset_path,
            _phase1_root_manifest_path_string(root_manifest, "archive_root", manifest_path=manifest_path),
        )
        loaded_archive_root = _archive_root_from_manifest_paths(loaded_source.get("manifest_paths") or ())
