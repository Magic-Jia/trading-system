...
- [x] Live-readiness smoke CLI 已可复核真实 artifact：从 corrected 30-trade chunk 输入生成 `live_readiness_gate.json` / `live_readiness_gate.md` / `trade_postmortem_summary.json` / `normalized_chunks/`，记录 `smoke_report` provenance，并复现 9 chunks / 30 trades / net PnL `-4,046.26` / `reject_for_live_promotion`。`trade_postmortem_summary.json` 应与 gate totals 对账一致，并把失败归因落到 failure taxonomy buckets。
