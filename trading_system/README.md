...
- 仓库现在提供 `deploy/systemd/trading-system-paper.service` 与 `deploy/systemd/trading-system-paper.timer` 作为最小可用的 paper 定时跑批模板；`python -m trading_system.run_cycle --mode paper` 会优先在当前 paper bucket 内自动补齐缺失的 `account_snapshot.json`、`market_context.json`、`derivatives_snapshot.json`，不会回退到旧的根目录快照路径。
